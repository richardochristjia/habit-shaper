...
const initialState: RelapseActionState = {};
...
function RelapseSubmitButton({ recorded }: { recorded: boolean }) {
  const { data, pending } = useFormStatus();
  const isThisIntent = data?.get("recorded") === String(recorded);
  const label = recorded ? "Record Relapse" : "Remove Relapse";
...
export function RelapseSection({
  ...
  progress: BreakHabitProgressView;
}) {
  const [selectedDay, setSelectedDay] = useState(progress.today);
  const [state, formAction] = useActionState(setRelapseAction, initialState);
  const [optimisticRelapseDays, updateOptimisticRelapse] = useOptimistic(
    progress.relapseDays,
    (currentDays, intent: RelapseIntent) => {
      if (!intent.recorded) {
  ...
  const isRecorded = optimisticRelapseDays.includes(selectedDay);
  const cleanStreak = calculateCleanStreak({
    startDate: progress.startDate,
    today: progress.today,
    relapseDays: optimisticRelapseDays,
  });
  ...
  return (
    <section
      ...
      aria-labelledby={`${progress.habitId}-relapse-heading`}
    >
      <div className="grid gap-4 rounded-panel border border-border bg-brand-soft p-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center sm:p-5">
        <div>
          <h3
            className="font-display text-2xl font-semibold leading-tight"
            id={`${progress.habitId}-relapse-heading`}
          >
            Relapses
          </h3>
          <p className="mt-1 text-sm text-muted-foreground">
            Record when the behaviour occurred or correct earlier history. Days
            without a Relapse are Clean Days.
          </p>
        </div>
      ...
      <form action={submitWithOptimism} className="mt-4 grid gap-3" noValidate>
        ...
        >
          {isRecorded
            ? "Relapse recorded for this Tracking Day."
            : "This Tracking Day is clean: no Relapse recorded."}
        </p>
        <FieldError
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          <RelapseSubmitButton recorded />
          <RelapseSubmitButton recorded={false} />
        </div>
        {state.success && (
